		...
		if (delay->check()) {
			...
			for (int i = 0; i < ChickIcon::count; i++) {
				...
				if (ChickIcon::chickList[i]->getFoodeaten() != 0 && ChickIcon::chickList[i]->getFoodeaten() % 2 == 0) {
					gameEggslist[eggsCounter] = new eggs(this, ChickIcon::chickList[i]->curr_pos, 20, 20);
					gameEggslist[eggsCounter]->draw();
					eggsCounter++;
				}
			}
			for (int i = 0; i < CowIcon::count; i++) {
				...
				if (CowIcon::cowList[i]->getFoodeaten() != 0 && CowIcon::cowList[i]->getFoodeaten() % 4 == 0) {
					gameMilklist[milkCounter] = new milk(this, CowIcon::cowList[i]->curr_pos, 20, 20);
					gameMilklist[milkCounter]->draw();
					milkCounter++;
				}
			}
			if (gameWolf) {
