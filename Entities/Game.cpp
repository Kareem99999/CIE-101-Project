...
{
	gameFarm->draw();
	if (gameFoodArea) { gameFoodArea->draw(); };
	gameWarehouse->draw();
}
	...
	do
	{
		if (delay->check()) {
			...
			for (int i = 0; i < ChickIcon::count; i++) {
				...
				ChickIcon::chickList[i]->isCollCh();
				gameFoodArea->Foodcounter -= ChickIcon::chickList[i]->TakeFood();
				cout << gameFoodArea->Foodcounter << endl;
			}
			for (int i = 0; i < CowIcon::count; i++) { 
				CowIcon::cowList[i]->moveStep();
				CowIcon::cowList[i]->isCollCo();
				gameFoodArea->Foodcounter -= CowIcon::cowList[i]->TakeFood();
				cout << gameFoodArea->Foodcounter << endl;
			}
			if (gameFoodArea->Foodcounter <= 0) {
				delete gameFoodArea;
			}
